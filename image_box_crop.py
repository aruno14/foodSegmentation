...
def applyMask(crop_img):
    crop_img_resized = crop_img.resize(SIZE)
    crop_img_resized = np.array(crop_img_resized)
    predictions = model.predict(np.array([crop_img_resized]))
    ...
    mask = np.where((mask > 0.8), 1.0, 0.2)

    #Resize
    mask = np.array(Image.fromarray(mask).resize(crop_img.size))
...
for idx, image_path in enumerate(["example/input-01.png", "data/images/DSC_0014.JPG", "data/imagesTest/20141029_215631_HDR.jpg", "data/images/table.jpg", "data/images/assiette-01.jpg", "data/images/choucroute-01.jpg", "data/images/repas-france-08.jpg"]):
