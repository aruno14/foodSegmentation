...
files_ds = files_ds.map(lambda x, y: (process_img(x), process_img(y, 1))).batch(1)
...
def showPrediction(imagePath:str, count=0):
    test_image = process_img("myData/images/assiette-01.jpg")
    predict = model.predict(np.asarray([test_image]))[0]

    fig, axes= plt.subplots(nrows=2, ncols=1)
    axes[0].imshow(test_image)
    axes[1].imshow(predict)
    plt.savefig("currentResult-"+str(count)+".png")
    plt.close()

class DisplayCallback(tf.keras.callbacks.Callback):
  def on_epoch_end(self, epoch, logs=None):
    showPrediction("myData/images/assiette-01.jpg", epoch)
    print ('\nSample Prediction after epoch {}\n'.format(epoch+1))

epochs = 5
print(files_ds)
history = model.fit(files_ds, epochs=epochs, callbacks=[DisplayCallback()])
